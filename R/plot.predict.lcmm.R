...
plot.predict.lcmm <- function(x,newdata,var.time,legend.loc="topright",na.action=1,legend,add=FALSE,confint=FALSE,...)
{
 ...
 {
  newdata<-newdata[sort.list(newdata[,var.time]),,drop=FALSE]
...
#  }
  ...
  if(x$ng==1)
  {
   #varpred <- diag(X1[,-1,drop=FALSE] %*% Vbeta %*% t(X1[,-1,drop=FALSE]))  #controler si l'ordre est le meme entre X1 et Vbeta
...
#  else nomsleg <- paste("class",1:x$ng,sep="")
  ...
  {
   do.call("matlines",c(dots.plot,list(x=X,y=Y,type=type1,lty=lty1,col=color)))
  }
