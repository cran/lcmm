plot.multlcmm <- function(x,...)
{
  ...
  else
  {
   if(x$conv==1|x$conv==2)
   {
    ...
    par(mfrow=c(1,2))
    do.call("plot",c(dots.plot,list(x=x$pred[,"pred_m"],y=x$pred[,"resid_m"],main=title1[1],xlab=xlab1[1],ylab=ylab1[1])))
    do.call("plot",c(dots.plot,list(x=x$pred[,"pred_ss"],y=x$pred[,"resid_ss"],main=title1[2],xlab=xlab1[2],ylab=ylab1[2])))  
   }
   else
   {
