...
!----------------------------------------------------------
      ...
      integer,save ::nprob,nvarprob,maxmes,nobs
...
! module pour la verification de type argument
	...
	double precision,intent(in)::vw
      ...
      subroutine maxt(maxtt,delta,m)

      double precision,intent(out)::maxtt
       integer,intent(in)::m
...
!C-------------------------------------------------------------
      ...
      implicit none
!c -Interface permettant la verification des type des arguments
      interface verif1
        module procedure marq98,deriva,searpas,dmfsd,valfpa
      end interface verif1

      interface verif2
        module procedure dsinv,dchole,maxt
      end interface verif2
...
!C      write(6,*) 'loglikelihood not improved '
         call maxt(maxtt,delta,m)
         if(maxtt.eq.0.D0) then
            vw=th
         else
            call maxt(maxtt,delta,m)
            vw=th/maxtt
...
!================ SUBROUTINES ================================
      ...
      subroutine hetmixlin(Y0,X0,Prior0, idprob0,idea0,idg0   &
          ,ns0,ng0,nv0,nobs0,nea0,nmes0,idiag0,nwg0   &
          ,npm0,b,vopt,vrais,ni,istop,gconv,ppi0,resid_m0,resid_ss0 &
          ,pred_m_g0,pred_ss_g0,pred_RE,convB,convL,convG,maxiter0)
